...
async function testConnection() {
  try {
    ...
    const tenantResult = await masterPool.query('SELECT database_url, slug FROM tenants WHERE slug = $1', ['bq_receipt']);
    ...
    if (tenantResult.rows.length > 0) {
      ...
      if (parseInt(countResult.rows[0].count) > 0) {
        const sampleResult = await tenantPool.query('SELECT * FROM receipts LIMIT 3');
